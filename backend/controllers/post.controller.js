...
export const createPost = async (req, res) => {
  try {
    ...
    if(!text && !img){
      return res.status(400).json({ error: 'Post must contain text or img' });
    }
...
export const getFollwingPosts = async (req, res) => {
  try {
    ...
    const feedposts = await Post.find({ user: { $in: follwing } }).sort({ createdAt: -1 }).populate({
      path: 'user',
      select: '-password'
    }).populate({
      path: 'comments.user',
    ...
    console.log("Error in getFollwingPosts:", error);
    res.status(500).json({ error: 'Server error' });
  }
}
